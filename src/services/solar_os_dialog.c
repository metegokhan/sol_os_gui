...
void solar_os_dialog_show_exit(const char *app_name, bool resumable)
{
    ...
    dialog_state.selected_button = resumable ? 1 : 1;
}
...
void solar_os_dialog_draw(solar_os_gfx_t *gfx)
{
    ...
    solar_os_gfx_text(gfx, dx + 12, dy + 18, "Exit Application");
    ...
    for (int i = 0; i < btn_count; i++) {
        ...
        const char *label = dialog_state.resumable ? labels_resumable[i] : labels_normal[i];
