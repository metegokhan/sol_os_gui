...
are saved, and `agent list` plus `agent resume ID` restore a selected transcript
after leaving the app or rebooting. `agent delete ID` removes one conversation
without changing provider credentials. Responses uses its saved provider
continuation ID; Chat Completions uses bounded local history. `agent ask`
