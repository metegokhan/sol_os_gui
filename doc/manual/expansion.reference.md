...
| ESP32-WROVER v3.0 | `uart0`: TX GPIO1, RX GPIO3 | I2C, SPI on `spi2`/`spi3`, UART on `uart1`/`uart2`, or 1-Wire, using GPIO13/GPIO32/GPIO33 | The rear SD slot uses the dedicated one-bit SDMMC host. SPI is limited to an output-only bus when all three free pins provide SCLK, MOSI, and CS. |
