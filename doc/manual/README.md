...
- [gameboy application](apps.md#gameboy) — Experimental original Game Boy (DMG) emulator for the Waveshare ESP32-S3-RLCD-4.2. It is included only in the retro flavor. The application loads a user-supplied ROM into PSRAM, renders its four shades as a 320x288 dithered image, and writes battery-backed cartridge RAM beside the ROM as a .sav file. Game Boy Color-only ROMs and ROMs larger than 4 MiB are rejected. The MiniGB APU backend renders pulse, wave, and noise channels through the shared SolarOS synth and audio services at the global speaker volume.
